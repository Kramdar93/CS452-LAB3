...
void createScaleMatrix4(float s, GLfloat out_m[4][4])
{
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 4; ++j)
...
void render() {
	...
	createTranslationMatrix4(currentX, currentY, currentZ, modelT);
	createRotationMatrixX4(currentRoll, modelX);
	createRotationMatrixY4(currentYaw, modelY);
	createRotationMatrixZ4(currentPitch, modelZ);
	createScaleMatrix4(currentS, modelS);
...
int main(int argc, char** argv) {
	...
	*/
	...
	parseFlatObjFile("test2.obj", &vertices, &vlen, &indexArray, &ilen);
